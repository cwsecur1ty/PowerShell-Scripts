# Run DCU-CLI -> Apply Updates 
function Update-DellWithoutReboot {
    ...
    Write-Output "Starting Dell Command Update without forced reboot..."

    # Run the Dell Command Update to apply updates without forcing reboot
    $process = Start-Process -FilePath $dcuCliPath -ArgumentList "/applyUpdates -reboot=disable -silent" -NoNewWindow -PassThru -Wait -ErrorAction Stop
    ...
    switch ($exitCode) {
        0 { Write-Output "Dell updates applied successfully without forced reboot." }
